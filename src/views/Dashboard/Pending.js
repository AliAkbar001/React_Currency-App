...
export default function Pending() {
  ...
  function FetchData(){
    axios.get(`${url_path}/sales/pending`).then(response => {
      let sum = 0
  ...
  const handleDateRangeData = ()=>{
    if(startD !== '' && endD !== ''){
      ...
      axios.post(`${url_path}/sales/pending/date-range`, {start: date1, end: date2}).then(response => {
        let sum = 0
        response.data.map((res)=> sum = sum + res.pending_amount)
        setTotalAmount(sum)
        setData(response.data)
  ...
  const handleSubmit = ()=>{
    ...
    if(check){
      ...
      axios.put(`${url_path}/sales/pending-payments`, {_id: payment._id, obj}).then(response => {
        if(response.data.acknowledged){
          ...
          onClose()
          toast({
            title: payment.username +  ' Pay ' + pendingAmount + ' PKR',
            status: 'success',
            duration: 9000,
            isClosable: true,
          })
  ...
  return (
    ...
    <Flex justifyContent={'space-between'} alignItems={'end'} gap={'0.5rem'}>
      <FormControl>
        <FormLabel>Search Pending Payments</FormLabel>
  ...
  <Modal
    ...
    >
      <ModalOverlay />
      <ModalContent>
          <ModalHeader>Receive Amount</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
